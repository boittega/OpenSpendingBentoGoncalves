d3.biHiSankey = function () {
    ...
    links = linkGroups.map(function (linkGroup) {
      return linkGroup.reduce(function (previousLink, currentLink) {
        return {
          ...
          "budget": previousLink.budget + currentLink.budget
        };
      });
    });
  }
  ...
  function computeNodeValues() {
    nodes.forEach(function (node) {
      ...
      node.budget = Math.max(
        d3.sum(visible(node.targetLinks), function (d) { return d.budget}),
        d3.sum(visible(node.sourceLinks), function (d) { return d.budget }));
      node.netFlow = Math.max(
        d3.sum(visible(node.targetLinks), value),
        d3.sum(visible(node.sourceLinks), value)); //d3.sum(visible(node.targetLinks), value); //d3.sum(visible(node.targetLinks), value) - d3.sum(visible(node.sourceLinks), value);
...
// função que calcula a posição X do node conforme o tamanho da tela
  ...
  function computeNodeYPositions(iterations) {
    ...
    //Calcula a altura permitira para o node.
        ...
        nodesByXPosition.forEach(function (nodes) {
            ...
            for (i = 0; i < n; ++i) {
                ...
                if (node.state === "contained") {
                    node.y = node.parent.yChild
                    node.parent.yChild += node.heightAllowance + nodeSpacing;
                    if (y0 < node.parent.yChild) y0 = node.parent.yChild;
                } else {
                    node.y = y0;
                    node.yChild = node.y;
                    y0 += node.heightAllowance + nodeSpacing;
                }
            }
            /*
